...
namespace VncSharp
{
    ...
    public class RfbProtocol
    {
        ...
        private readonly ushort[,] mapEntries = new ushort[256, 3];
        ...
        /// <param
        ///     name="port" >
        ///     The Port number on which to connect.  Usually this will be 5900, except in the case that the VNC Host is running on a different Display, in which case the Display
        ///     number should be added to 5900 to determine the correct port.
        /// </param>
        ...
        public void ReadProtocolVersion()
        {
            ...
            else throw new NotSupportedException("Only versions 3.3, 3.7, and 3.8 of the RFB Protocol are supported.");
        }
        ...
        public byte[] ReadSecurityTypes()
        {
            ...
            if (verMinor == 3) types = new[] { (byte)reader.ReadUInt32() };
            else
            {
        ...
        public void WriteSetEncodings(uint[] encodings)
        {
            ...
            for (var i = 0; i < encodings.Length; i++) writer.Write(encodings[i]);
        ...
        /// <summary>
        ///     BigEndianBinaryReader is a wrapper class used to read .NET integral types from a Big-Endian stream.  It inherits from BinaryReader and adds Big- to Little-Endian
        ///     conversion.
        /// </summary>
        protected sealed class BigEndianBinaryReader : BinaryReader
        {
            ...
            private void FillBuff(int totalBytes)
            {
                ...
                do
                {
                    ...
                    if (n == 0) throw new IOException("Unable to read next byte(s).");
        ...
        /// <summary>
        ///     BigEndianBinaryWriter is a wrapper class used to write .NET integral types in Big-Endian order to a stream.  It inherits from BinaryWriter and adds Little- to
        ...
        public sealed class ZRLECompressedReader : BinaryReader
        {
            ...
            public void DecodeStream()
            {
                ...
                var buff = new byte[4];
                if (BaseStream.Read(buff, 0, 4) != 4) throw new Exception("ZRLE decoder: Invalid compressed stream size");

                // BigEndian to LittleEndian conversion
                var compressedBufferSize = buff[3] | buff[2] << 8 | buff[1] << 16 | buff[0] << 24;
                if (compressedBufferSize > 64 * 1024 * 1024) throw new Exception("ZRLE decoder: Invalid compressed data size");

                // Decode stream
                var pos = 0;
                while (pos++ < compressedBufferSize) zlibDecompressedStream.WriteByte(BaseStream.ReadByte());
