...
namespace VncSharp
{
    ...
    public class VncClient
    {
        ...
        {
            get { return inputPolicy != null && inputPolicy is VncViewInputPolicy; }
        }
        ...
        /// <param
        ///     name="viewOnly" >
        ///     True if mouse/keyboard events are to be ignored.
        /// </param>
        ...
        public bool Connect(string host, int display, int port, bool viewOnly)
        {
            ...
            if (viewOnly) inputPolicy = new VncViewInputPolicy(rfb);
            else inputPolicy = new VncDefaultInputPolicy(rfb);
        ...
        /// <summary>
        ///     Connect to a VNC Host and determine which type of Authentication it uses. If the host uses Password Authentication, a call to Authenticate() will be required.
        ///     Default Display and Port numbers are used.
        /// </summary>
        /// <param
        ///     name="host" >
        ///     The IP Address or Host Name of the VNC Host.
        ...
        /// <summary>
        ///     Connect to a VNC Host and determine which type of Authentication it uses. If the host uses Password Authentication, a call to Authenticate() will be required. The
        ///     Port number is calculated based on the Display.
        /// </summary>
        /// <param
        ///     name="host" >
        ///     The IP Address or Host Name of the VNC Host.
        ...
        public bool Authenticate(string password)
        {
            ...
            if (securityType == 2) PerformVncAuthentication(password);
        ...
        protected byte[] EncryptChallenge(string password, byte[] challenge)
        {
            ...
            if (password.Length >= 8) Encoding.ASCII.GetBytes(password, 0, 8, key, 0);
            else Encoding.ASCII.GetBytes(password, 0, password.Length, key, 0);
        ...
        protected void OnConnectionLost()
        {
                ...
                ConnectionLost.Target is Control)
            {
                ...
                if (target != null) target.Invoke(ConnectionLost, this, EventArgs.Empty);
                else ConnectionLost(this, EventArgs.Empty);
            }
        }
        ...
        protected void OnServerCutText()
        {
                ...
                ServerCutText.Target is Control)
            {
                ...
                if (target != null) target.Invoke(ServerCutText, this, EventArgs.Empty);
                else ServerCutText(this, EventArgs.Empty);
            }
        }
        ...
        ///     TRUE if the entire screen should be refreshed, FALSE if only a partial region needs updating.
        /// </param>
        /// <remarks>
        ///     RequestScreenUpdate needs to be called whenever the client screen needs to be updated to reflect the state of the remote
