...
namespace VncSharp
{
    ...
    /// <summary>
    ///     When connecting to a VNC Host, a password will sometimes be required.  Therefore a password must be obtained from the user.  A default Password dialog box is
    ///     included and will be used unless users of the control provide their own Authenticate delegate function for the task.  For example, this might pull a password from a
    ///     configuration file of some type instead of prompting the user.
    /// </summary>
    ...
    public class RemoteDesktop : Panel
    {
        ...
        /// <summary>
        ///     Points to a Function capable of obtaining a user's password.  By default this means using the PasswordDialog.GetPassword() function; however, users of RemoteDesktop
        ///     can replace this with any function they like, so long as it matches the delegate type.
        /// </summary>
        ...
        ///     <see
        ///         cref="VncSharp.RemoteDesktop.IsConnected" />
        ///     .
        /// </exception>
        /// <exception
        ///     cref="System.NullReferenceException" >
        ...
        public void Authenticate(string password)
        {
            ...
            if (vnc.Authenticate(password)) Initialize();
        ...
        /// <param
        ///     name="viewOnly" >
        ///     Determines whether mouse and keyboard events will be sent to the host.
        /// </param>
        /// <exception
        ///     cref="System.ArgumentNullException" >
        ///     Thrown if host is null.
        ...
        /// <param
        ///     name="text" >
        ///     The text to put in the server's clipboard.
        /// </param>
        ...
        /// <param
        ///     name="keys" >
        ///     SpecialKeys is an enumerated list of supported keyboard combinations.
        /// </param>
        ...
        {
            if (scaled) desktopPolicy = new VncScaledDesktopPolicy(vnc, this);
            else desktopPolicy = new VncClippedDesktopPolicy(vnc, this);
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                if (state != RuntimeState.Disconnected) Disconnect();
        ...
        /// <exception
        ///     cref="System.InvalidOperationException" >
        ///     Thrown if the RemoteDesktop control is not already in the Connected state.
        /// </exception>
        ...
        {
            if (ConnectComplete != null) ConnectComplete(this, e);
        }
        ...
        protected override void OnMouseMove(MouseEventArgs mea)
        {
            ...
            if (IsConnected)
            {
                ...
                if (adjusted.Contains(PointToClient(MousePosition))) UpdateRemotePointer();
            }
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (!DesignMode)
            {
                switch (state)
                {
                    ...
                    default:
                        // Sanity check
                        throw new NotImplementedException(string.Format("RemoteDesktop in unknown State: {0}.", state));
                }
            }
            else
            {
        ...
        protected override void OnResize(EventArgs eventargs)
        {
            ...
            while (parent != null)
            {
                if (parent is Form)
                {
                    ...
                    if (form.WindowState == FormWindowState.Maximized) form.Invalidate();
                    parent = null;
                }
        ...
        /// <param
        ///     name="sender" >
        ///     The VncClient object that raised the event.
        /// </param>
        /// <param
        ///     name="e" >
        ...
        /// <param
        ///     name="connected" >
        ///     True if the connection must be established, otherwise False.
        /// </param>
        /// <exception
        ///     cref="System.InvalidOperationException" >
        ///     Thrown if the RemoteDesktop control is in the wrong state.
        /// </exception>
        private void InsureConnection(bool connected)
        {
            ...
            if (connected)
            {
                ...
                if (state != RuntimeState.Connected && state != RuntimeState.Disconnecting) throw new InvalidOperationException("RemoteDesktop must be in Connected state before calling methods that require an established connection.");
            }
            else
            {
                // disconnected
                ...
                if (state != RuntimeState.Disconnected && state != RuntimeState.Disconnecting) throw new InvalidOperationException("RemoteDesktop cannot be in Connected state when calling methods that establish a connection.");
            }
        }
        ...
        /// <param
        ///     name="keys" >
        ///     An array of keysym values representing keys to press/release.
        /// </param>
        /// <param
        ///     name="release" >
        ///     A boolean indicating whether the keys should be Pressed and then Released.
        /// </param>
        private void PressKeys(uint[] keys, bool release)
        {
            Debug.Assert(keys != null, "keys[] cannot be null.");

            for (var i = 0; i < keys.Length; ++i) vnc.WriteKeyboardEvent(keys[i], true);

            if (release)
            {
                // Walk the keys array backwards in order to release keys in correct order
                for (var i = keys.Length - 1; i >= 0; --i) vnc.WriteKeyboardEvent(keys[i], false);
            }
        }
        ...
        private void UpdateRemotePointer()
        {
            ...
            if (!DesignMode && IsConnected)
            {
                ...
                if (adjusted.X < 0 || adjusted.Y < 0) throw new Exception();
