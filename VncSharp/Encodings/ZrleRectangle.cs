...
namespace VncSharp.Encodings
{
    ...
    public sealed class ZrleRectangle : EncodedRectangle
    {
        ...
        private static readonly int[] palette = new int[128];
        private static readonly int[] tileBuffer = new int[TILE_WIDTH * TILE_HEIGHT];
        ...
        public override void Decode()
        {
            ...
            for (var ty = 0; ty < rectangle.Height; ty += TILE_HEIGHT)
            {
                ...
                for (var tx = 0; tx < rectangle.Width; tx += TILE_WIDTH)
                {
                    ...
                    if ((subencoding >= 17 && subencoding <= 127) || subencoding == 129) throw new Exception("Invalid subencoding value");

                    var isRLE = (subencoding & 128) != 0;
                    var paletteSize = subencoding & 127;

                    // Fill palette
                    for (var i = 0; i < paletteSize; i++) palette[i] = preader.ReadPixel();
