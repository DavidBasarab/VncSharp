...
namespace VncSharp.Encodings
{
    ...
    public abstract class EncodedRectangle : IDesktopUpdater
    {
        ...
        public EncodedRectangle(RfbProtocol rfb, Framebuffer framebuffer, Rectangle rectangle, int encoding)
        {
            ...
            {
                case 32:
                    if (encoding == RfbProtocol.ZRLE_ENCODING) preader = new CPixelReader(reader, framebuffer);
                    else preader = new PixelReader32(reader, framebuffer);
                    break;
                case 16:
        ...
        public virtual unsafe void Draw(Bitmap desktop)
        {
            ...
            try
            {
                ...
                for (var y = 0; y < rectangle.Height; ++y)
                {
                    ...
                    for (var x = 0; x < rectangle.Width; ++x) *pInt++ = framebuffer[row + x];
        ...
        protected void FillRectangle(Rectangle rect, int colour)
        {
            ...
            {
                for (var x = 0; x < rect.Width; ++x) framebuffer[ptr++] = colour; // colour every pixel the same
        ...
        protected void FillRectangle(Rectangle rect, int[] tile)
        {
            ...
            for (var y = 0; y < rect.Height; ++y)
            {
                for (var x = 0; x < rect.Width; ++x) framebuffer[ptr++] = tile[idx++];
                ptr += offset; // advance to next row within pixels
            }
        }
        ...
        protected void FillRectangle(Rectangle rect)
        {
            ...
            for (var y = 0; y < rect.Height; ++y)
            {
                for (var x = 0; x < rect.Width; ++x) framebuffer[ptr++] = preader.ReadPixel(); // every pixel needs to be read from server
                ptr += offset; // advance to next row within pixels
            }
        }
    }
